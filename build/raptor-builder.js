...
function getModules() {
    if (addedModules.length === 0) {
        ...
        lib.writeDebug('Added modules: \n    ' + addedLog.alphanumSort().join(', \n    ') + '\n');
        lib.writeDebug('Skipped modules: \n    ' + skippedLog.alphanumSort().join(', \n    ') + '\n');
        lib.writeDebug('Excluded modules: \n    ' + excludedLog.alphanumSort().join(', \n    ') + '\n');
    }
    return addedModules;
};
...
function compressCSS(inputFile, outputFile) {
    ...
    var cssMin = new CleanCSS(options).minify(css).styles;
